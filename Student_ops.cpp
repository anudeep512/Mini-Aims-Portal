// In this file, we are going to define all the Operations which we use for Student class
...
void Student::reset_course_registration_list(const string& a, const vector<string>& b)
{
    ofstream dfg;
...
void Student::store_my_courses_in_file_for_deletion(const vector<string>& A)
{
    ofstream fp;
    string D;
...
void Student::store_my_courses_in_file(const vector<string>& A)
{
    ofstream fp;
    string D;
    D = roll_number + "_courses_registered.txt";
...
void Student::register_for_courses()
{
    ...
    while (getline(see_course_list, s))
        cout << s << endl; // By seeing this list, the student should register only for the courses for which the course grades are not submitted yet
...
// This function is used to check grades of a course
